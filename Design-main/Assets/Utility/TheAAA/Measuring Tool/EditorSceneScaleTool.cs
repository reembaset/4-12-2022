...
public class EditorSceneScaleTool : MonoBehaviour
{
    ...
    [SerializeField] Color scaleObjectScaleColor = Color.magenta; // Castom resizable scale color
    public DistanceType sizeType; // see enum
    
    [SerializeField] GUIStyle textStyle; // scale text style
    ...
    {
        int ft = 0;
        for (float i = 0; ft <= scaleSize; i+= feetToMeter)
        {
    ...
    {
        int inch = 0;
        for (float i = 0; inch <= scaleSize; i += InchToMeter)
        {
    ...
    {
        int cm = 0;
        for (float i = 0; cm <= scaleSize; i += cmToMeter)
        {
    ...
    {
        int yd = 0;
        for (float i = 0; yd <= scaleSize; i += ydToMeter)
        {
